...
def get_gemini_api_key() -> str:
    ...
    """
    with open(os.path.join(ROOT_DIR, "config.json"), "r") as file:
        configured = json.load(file).get("gemini_api_key", "")
        return configured or os.environ.get("GEMINI_API_KEY", "AIzaSyBX5gIZolt0gyJcp0IyHzKqBFD3h-ixTjc")
...
def get_gemini_model() -> str:
    ...
    """
    with open(os.path.join(ROOT_DIR, "config.json"), "r") as file:
        configured = json.load(file).get("gemini_model", "")
        # Fallback para o modelo padrão da família 3.x Flash Lite
        return configured or os.environ.get("GEMINI_MODEL", "gemini-3.1-flash-lite-preview")
